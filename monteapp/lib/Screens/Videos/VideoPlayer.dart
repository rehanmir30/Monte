...
class VideoPlayer extends StatefulWidget {
  ...
  const VideoPlayer(this.videoUrls, {Key? key}) : super(key: key);
...
class _VideoPlayerState extends State<VideoPlayer> {
  ...
  Future<void> _initializeController() async {
    ...
    final savedPosition = prefs.getInt('video_position') ?? 0;
    final currentIndex = prefs.getInt('video_index') ?? 0;

    _controller = vp.VideoPlayerController.network(widget.videoUrls[currentIndex])
      ..initialize().then((_) {
        setState(() {
          _isPlaying = true;
    ...
    _controller.addListener(() async {
      ...
      await prefs.setInt('video_index', currentIndex);
      if (_controller!.value.position >= _controller!.value.duration) {
       await prefs.setInt("video_position", 0);
       await prefs.setInt("video_index", 0);
      }
    });
  }
            ...
            children: <Widget>[
              ...
              SizedBox(
                ...
                child: vp.VideoProgressIndicator(
                  _controller,colors: vp.VideoProgressColors(playedColor: kPink,bufferedColor: kYellow.withOpacity(0.2)),
              ...
              IconButton(
                icon: Icon(Icons.speed,color:isFast?Colors.white:kPink),
                onPressed: () {
                  setState(() {
                    isFast=false;
                  });
              ...
              IconButton(
                icon: Icon(Icons.speed,color:isFast?kPink:Colors.white),
                onPressed: () {
                  setState(() {
                    isFast=true;
                  });
